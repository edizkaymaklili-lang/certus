...
`delete_file` matches the packaged default policy's `critical_tools` list, so it requires approval before it runs; wire your own approval channel (Slack, a ticketing system, a CLI prompt) via `approval_callback=`. See [`examples/quickstart.py`](examples/quickstart.py) for a runnable end-to-end demo, including a denylisted call being blocked outright and decision-only "gateway mode" usage.
...
src/certus/
  ...
  proxy/           Execution interception layer.
    ...
    approval.py      Pluggable human-in-the-loop approval (CLI, custom callback, or your own)
  sdk/
  ...
  edge/            Edge-AI MLOps pipeline (Colab <-> orchestrator bridge).
    ...
    quantize.py      EdgePackager — PyTorch -> ONNX -> INT8 ONNX / INT8 TFLite

examples/
  quickstart.py                 Guardrail SDK end-to-end demo
  config.json                   Sample TrainingConfig
  colab_training_template.py    Standalone Colab training + edge-packaging script
...
`suggest_next_config` is a deterministic rule set (overfit detection, plateau detection, failed-run retry) an orchestrating agent can call, inspect, and override — never a hidden decision the pipeline makes on its own. Run [`examples/colab_training_template.py`](examples/colab_training_template.py) end-to-end (`pip install "certus-ai[edge]"` first) to see the full loop: `config.json` in, trained model out, `metrics.json` + INT8 ONNX/TFLite artifacts + `manifest.json` written to disk.
